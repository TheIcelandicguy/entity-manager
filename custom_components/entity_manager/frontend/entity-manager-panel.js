...
const _emBaseUrl = _emScripts.length > 0 
  ...
  : '/local/community/entity_manager/';
...
if (!document.querySelector('link[href*="entity-manager-panel.css"]')) {
  ...
  cssLink.href = _emBaseUrl + 'entity-manager-panel.css';
...
class EntityManagerPanel extends HTMLElement {
  ...
  _handleKeyboard(e) {
    ...
    // Escape: Close any open dialog/dropdown
    if (e.key === 'Escape') {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'r') {
      e.preventDefault();
      this.loadData();
      this._showToast('Refreshing data...', 'info');
    }
    ...
    if (e.key === 'Delete' && this.selectedEntities.size > 0) {
      e.preventDefault();
      this._disableSelectedEntities();
    }
    
    // Ctrl/Cmd + Shift + R: Open bulk rename
    ...
    // Ctrl/Cmd + G: Toggle smart groups
    ...
    const { overlay, closeDialog } = this.createDialog({
      ...
      contentHtml: `
        <div class="shortcuts-list">
          ...
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>A</kbd> <span>Select all visible</span></div>
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>A</kbd> <span>Deselect all</span></div>
          ...
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>R</kbd> <span>Refresh data</span></div>
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>B</kbd> <span>Toggle sidebar</span></div>
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>G</kbd> <span>Toggle smart groups</span></div>
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>E</kbd> <span>Export config</span></div>
          <div class="shortcut-item"><kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>R</kbd> <span>Bulk rename</span></div>
          <div class="shortcut-item"><kbd>Delete</kbd> <span>Disable selected</span></div>
          <div class="shortcut-item"><kbd>Esc</kbd> <span>Close dialogs</span></div>
          <div class="shortcut-item"><kbd>?</kbd> <span>Show this help</span></div>
        </div>
      `,
      actionsHtml: `<button class="btn btn-primary">Got it</button>`
    });
  ...
  _showHelpGuide() {
    const { overlay, closeDialog } = this.createDialog({
      ...
      `,
      actionsHtml: `
        <button class="btn btn-secondary" id="show-shortcuts-btn">Keyboard Shortcuts</button>
        <button class="btn btn-primary">Close</button>
      `
    });
    
    overlay.querySelector('.btn-primary').addEventListener('click', closeDialog);
    overlay.querySelector('#show-shortcuts-btn').addEventListener('click', () => {
      closeDialog();
      this._showKeyboardShortcutsHelp();
    });
  }
    ...
    return `
      <div class="em-sidebar ${this.sidebarCollapsed ? 'collapsed' : ''}">
        ...
        <div class="sidebar-section">
          ...
          <div class="sidebar-item" data-action="keyboard-shortcuts">
            <span class="icon">⌨️</span>
  ...
  render() {
    ...
    // CSS is loaded externally via entity-manager-panel.css
