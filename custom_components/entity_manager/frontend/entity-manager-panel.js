...
class EntityManagerPanel extends HTMLElement {
  constructor() {
    ...
    this.labelViewMode = localStorage.getItem('em-label-view-mode') || 'entity'; // 'entity' | 'device'
    ...
    return `
      <div class="em-sidebar ${this.sidebarCollapsed ? 'collapsed' : ''}">
        ...
        <div class="sidebar-section ${this.sidebarOpenSections.has('labels') ? '' : 'section-collapsed'}" id="labels-section">
          ...
          <div class="sidebar-item ${this.labelViewMode === 'device' ? 'active' : ''}" data-label-mode="device">
            <span class="icon">🔌</span>
            <span class="label">By Device</span>
          </div>
          <div class="sidebar-item ${this.labelViewMode === 'entity' ? 'active' : ''}" data-label-mode="entity">
            <span class="icon">📋</span>
            <span class="label">By Entity</span>
          </div>
  ...
  _handleSidebarItemAction(item) {
    ...
    if (action === 'activity-log') {
      ...
      this.updateView();
      this.updateDomainOptions();
    } else if (item.dataset.labelMode) {
      this.labelViewMode = item.dataset.labelMode;
      localStorage.setItem('em-label-view-mode', this.labelViewMode);
      this.expandedSidebarLabels.clear();
      this._reRenderSidebar();
      this._loadAndDisplayLabels();
  ...
  attachIntegrationListeners() {
    ...
    this.content.querySelectorAll('.smart-group-header').forEach(header => {
      header.addEventListener('click', (e) => {
        if (e.target.closest('.smart-group-assign-all-btn')) return;
    ...
    // Smart group assign-all buttons (Unassigned / No Floor / No Area)
    this.content.querySelectorAll('.smart-group-assign-all-btn').forEach(btn => {
      btn.addEventListener('click', async e => {
        e.stopPropagation();
        const groupKey = btn.dataset.groupKey;
        const groups = this._getSmartGroups();
        const entities = groups[groupKey] || [];
        const deviceIds = [...new Set(entities.map(en => en.device_id).filter(Boolean))];
        const noDeviceEntities = entities.filter(en => !en.device_id);
        this._showAreaPickerDialog(`Assign ${entities.length} entities to area`, async areaId => {
                  ...
                  return `
                    <div class="entity-list-item" style="padding:10px 12px">
                      ...
                      <div style="font-size:0.88em;margin-top:5px;display:flex;gap:16px;flex-wrap:wrap;opacity:0.9">
                        ...
                        <span>Latest: <strong style="color:var(--em-success)">${this._escapeHtml(latest)}</strong></span>
                        ${releaseUrl ? `<a href="${this._escapeAttr(releaseUrl)}" target="_blank" rel="noopener" style="color:var(--em-primary)">Release notes</a>` : ''}
                      </div>
                    </div>
                  `;
                }).join('')}
              ...
              const body = items.map(e => `
                <div class="entity-list-item" style="padding:10px 12px">
                  <div class="entity-list-row" style="flex-wrap:wrap;align-items:center">
                    ...
                    ${e.is_disabled ? '<span style="background:var(--em-danger);color:#fff;padding:1px 6px;border-radius:10px;font-size:0.78em;margin-left:6px">disabled</span>' : ''}
                  </div>
                </div>
              `).join('');
